...
Each of these should be performed for iOS and Android.
...
Test 1
Title:
Switch between listening modes while listening to audio on iOS device.
Preconditions:
Earbuds (HW version: X, FW version: Y) are connected to iOS device (device model: A, iOS version: B). Earbuds battery is above 5 \%. Earbuds are in Transparency Mode (which is correctly displayed on the iOS app). Audio is playing in earbuds from iOS device (earbuds are in A2DP state).
Steps:
...
Test 2
Title:
Disconnect and reconnect earbuds to Android device and check listening mode.
Preconditions:
Earbuds (HW version: X, FW version: Y) are connected to Android device (device model: A, Android version: B). Earbuds battery is above 5 \%. Earbuds are in Transparency Mode (which is correctly displayed on the Android app).
Steps:
...
Bug report
Title:
Updating listening mode on earbuds is late / unsuccessful
(*) Priority: High
Environment:
Earbuds (HW version: X, FW version: Y), iOS/Android mobile device (device model: A, iOS/Android version: B)
Preconditions:
Earbuds are connected to the mobile device and inserted in tester's ears. Earbuds battery is above 5 \% (assumed). Earbuds are in Noise Cancellation Mode (which is correctly displayed on the mobile app).
Steps:
...
(*) I would also stress-test the feature in an environment with high RF interference.
...
Rcvd Mode Change
Rcvd Connection oriented channel
Sent Vendor Command 0x01C1
Sent Connection oriented channel
Rcvd Command Complete (Vendor Command 0x01C1)
Sent Vendor Command 0x01C5
Sent Vendor Command 0x01C1
Sent Vendor Command 0x01C2
Sent Connection oriented channel
Rcvd Connection oriented channel
Rcvd Command Complete (Vendor Command 0x01C5)
Rcvd Command Complete (Vendor Command 0x01C1)
Rcvd Command Complete (Vendor Command 0x01C2)
Rcvd Connection oriented channel
Rcvd Connection oriented channel
Rcvd Unknown 0xeb
Rcvd Unknown 0xeb
Rcvd Unknown 0xeb
Rcvd Unknown 0xeb
Sent Vendor Command 0x01C4

a bunch of HCI H4 "Sent Unknown HCI packet type 0x41/0x40 in between and the occasional HCI EVT "Rcvd Number of Completed Packets".
...
HCI_CMD (host ↔ controller (only visible on the device being snooped):
...
L2CAP (the transport layer carrying everything else):
Device 3 has no L2CAP messages
Device 2 exchanges L2CAP messages with other earpod (sometimes source and destination have same MAC address?)
